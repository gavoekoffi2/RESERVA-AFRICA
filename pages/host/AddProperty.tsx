...
const AddProperty: React.FC = () => {
  ...
  const handlePublish = async () => {
      ...
      if (user.verificationStatus !== 'verified') {
          addNotification('error', 'Vous devez être vérifié pour publier une annonce.');
          setStep(-1);
          return;
      }
      ...
      // Automatically upgrade user to HOST role if they aren't one yet
      if (user.role === 'GUEST') {
  ...
  const handleStartVerification = () => {
      ...
      setTimeout(() => {
          ...
          // Simulate auto-approval for the demo flow convenience
          setTimeout(() => {
              if (user) {
                  updateUser({ verificationStatus: 'verified' });
                  addNotification('success', 'Identité validée avec succès !');
              }
          }, 3000);
      }, 1500);
  };
  ...
  if (step === -1 && user?.verificationStatus !== 'verified') {
      return (
          <div className="min-h-screen bg-white dark:bg-[#0a0f18] flex flex-col font-display animate-reveal">
              <div className="h-20 px-8 flex items-center justify-between border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white/80 dark:bg-[#0a0f18]/80 backdrop-blur-md z-[110]">
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-6 text-center max-w-2xl mx-auto">
                  ...
                  <h1 className="text-3xl md:text-5xl font-black text-gray-900 dark:text-white mb-6 tracking-tighter leading-tight">Vérification de sécurité.</h1>
                  
                  {user?.verificationStatus === 'unverified' ? (
                      <>
                        <p className="text-lg text-gray-500 mb-10 leading-relaxed">
                          Pour devenir hôte sur Reseva Africa, nous devons confirmer votre identité. 
                          C'est essentiel pour maintenir une communauté de confiance.
                        </p>
                        <div className="w-full max-w-sm space-y-4">
                            ...
                            >
                                {isVerifying ? <span className="size-6 border-4 border-white border-t-transparent rounded-full animate-spin"></span> : 'Commencer la vérification'}
                            </button>
                      ...
                      <>
                        <div className="bg-yellow-50 dark:bg-yellow-900/20 p-8 rounded-[40px] border-2 border-yellow-100 dark:border-yellow-900/30 mb-8 w-full">
                            <div className="flex flex-col items-center gap-4 text-yellow-800 dark:text-yellow-200">
                                <span className="material-symbols-outlined text-4xl animate-pulse">hourglass_top</span>
                                <div>
                                    <p className="font-black text-xl">Dossier en cours d'analyse</p>
                                    <p className="opacity-80 mt-1 font-medium">Nos agents valident vos documents.</p>
                                </div>
                            </div>
                        </div>
                        <p className="text-gray-500 mb-12 font-medium">Cette étape prend généralement quelques minutes. Vous serez redirigé automatiquement.</p>
                        <button onClick={() => navigate('/')} className="px-12 py-5 bg-black dark:bg-white text-white dark:text-black font-black rounded-3xl shadow-xl uppercase tracking-widest text-sm btn-active-scale">Retour à l'accueil</button>
                      </>
                  )}
              </div>
          </div>
  ...
  const renderStepContent = () => {
      switch(step) {
                ...
                <div className="flex flex-col lg:flex-row h-full w-full max-w-[1440px] mx-auto animate-reveal overflow-hidden rounded-[40px] bg-white dark:bg-[#1a202c] shadow-2xl">
                    <div className="lg:flex-1 bg-black flex flex-col justify-center p-12 md:p-20 text-white relative">
                        ...
                        <div className="relative z-10 space-y-4">
                            ...
                            <h1 className="text-4xl md:text-6xl font-black tracking-tighter leading-[1]">Devenez hôte en <br/> quelques étapes.</h1>
                        </div>
                    </div>
          ...
          case 2: // Category
              return (
                  <div className="animate-reveal max-w-3xl mx-auto w-full px-6">
                      <h2 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white mb-10 tracking-tighter">Lequel de ces termes décrit le mieux votre {formData.type.toLowerCase()} ?</h2>
                      <div className="grid grid-cols-2 gap-4">
                          {['Villa', 'Appartement', 'Lodge', 'Chambre', 'Hôtel', 'Maison'].map(cat => (
                              <button 
                                key={cat}
                                onClick={() => setFormData({...formData, category: cat})}
                                className={`p-6 rounded-3xl border-2 transition-all flex items-center gap-5 hover:border-black dark:hover:border-white ${formData.category === cat ? 'border-black dark:border-white bg-black/5 dark:bg-white/5' : 'border-gray-100 dark:border-gray-800'}`}
                              >
          ...
          case 3: // Location
              return (
                  <div className="animate-reveal max-w-2xl mx-auto w-full px-6">
                      <h2 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white mb-4 tracking-tighter">Où se situe-t-il ?</h2>
                      <p className="text-gray-500 mb-10 font-medium">Une adresse précise aide les voyageurs à mieux planifier.</p>
                      <div className="space-y-6">
                         <div className="relative">
                             ...
                             <input 
                                ...
                                /* FIX: Corrected undefined setDestination to update formData.location correctly via setFormData */
          ...
          case 5: // Amenities
              return (
                  <div className="animate-reveal max-w-3xl mx-auto w-full px-6">
                      <h2 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white mb-4 tracking-tighter">Quels équipements offrez-vous ?</h2>
                      <p className="text-gray-500 mb-10 font-medium">Cochez tout ce qui s'applique.</p>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {['Wifi', 'Climatisation', 'Cuisine', 'Piscine', 'Parking', 'TV', 'Lave-linge', 'Espace Travail'].map(amenity => (
                              <button 
                                key={amenity}
          ...
          case 6: // Photos
              return (
                  <div className="animate-reveal max-w-3xl mx-auto w-full px-6">
                      <h2 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white mb-10 tracking-tighter">Mettez votre bien en valeur</h2>
                      <div className="grid grid-cols-2 gap-4">
                          <div className="col-span-full aspect-video bg-gray-50 dark:bg-gray-800 border-4 border-dashed border-gray-200 dark:border-gray-700 rounded-[40px] flex flex-col items-center justify-center gap-4 group cursor-pointer hover:bg-primary/5 transition-all">
                              ...
                              <p className="font-black text-gray-400 group-hover:text-primary transition-colors">Charger 5 photos minimum</p>
                          </div>
          ...
          case 7: // Title
              return (
                  <div className="animate-reveal max-w-2xl mx-auto w-full px-6">
                      ...
                      <p className="text-gray-500 mb-10 font-medium">Soyez précis sur l'atout majeur de votre offre.</p>
                      <textarea 
                        autoFocus
                        value={formData.title}
                        onChange={(e) => setFormData({...formData, title: e.target.value})}
                        className="w-full h-40 p-8 rounded-[32px] border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50 outline-none focus:border-primary transition-all font-black text-2xl placeholder:text-gray-300 leading-tight"
                        placeholder="Ex: Villa Emeraude avec piscine à Baguida..."
                        maxLength={60}
                      ></textarea>
          ...
          case 8: // Description
              return (
                  <div className="animate-reveal max-w-2xl mx-auto w-full px-6">
                      ...
                      <p className="text-gray-500 mb-10 font-medium">Parlez du quartier, des services et de l'expérience.</p>
                      <textarea 
                        autoFocus
                        value={formData.description}
                        onChange={(e) => setFormData({...formData, description: e.target.value})}
                        className="w-full h-60 p-8 rounded-[32px] border-2 border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-800/50 outline-none focus:border-primary transition-all font-bold text-lg leading-relaxed placeholder:text-gray-300"
                        placeholder="Qu'est-ce qui rend votre logement unique ?..."
                      ></textarea>
                  </div>
              );
          case 9: // Pricing
              return (
                  <div className="animate-reveal max-w-2xl mx-auto w-full px-6 text-center">
                      ...
                      <p className="text-gray-500 mb-12 font-medium">Prix par nuit. Vous pourrez l'ajuster selon les saisons.</p>
                      <div className="flex items-center gap-6 bg-gray-50 dark:bg-gray-800 p-10 rounded-[48px] border-2 border-gray-100 dark:border-gray-700 w-full justify-center shadow-inner">
                          <span className="text-4xl md:text-6xl font-black text-gray-200">F</span>
                          <input 
                            type="number" 
                            autoFocus
                            value={formData.price || ''}
                            onChange={(e) => setFormData({...formData, price: Number(e.target.value)})}
                            className="bg-transparent border-none outline-none font-black text-5xl md:text-7xl text-gray-900 dark:text-white text-center w-full max-w-[300px]"
                            placeholder="0"
                          />
                      </div>
                      <div className="mt-10 p-6 bg-primary/5 rounded-3xl border border-primary/10 flex justify-between items-center text-left">
                          ...
                          <span className="text-[10px] text-gray-400 font-bold max-w-[120px]">Après commission (15%)</span>
                      </div>
                  </div>
              );
          case 10: // Review
                      ...
                      <div className="bg-white dark:bg-[#1a202c] rounded-[48px] border-2 border-gray-100 dark:border-gray-800 shadow-2xl overflow-hidden flex flex-col md:flex-row group">
                          ...
                          <div className="p-10 flex flex-col justify-between flex-1">
                              <div>
                                  ...
                                  <h3 className="text-2xl font-black text-gray-900 dark:text-white leading-tight mb-4">{formData.title || 'Votre annonce sans titre'}</h3>
                                  <p className="text-sm text-gray-500 line-clamp-3">{formData.description || 'Vérifiez bien que toutes les informations sont correctes avant de soumettre.'}</p>
                              </div>
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0a0f18] flex flex-col font-display selection:bg-primary selection:text-white">
      ...
      {/* Form Content Area */}
      <div className="flex-1 flex flex-col items-center justify-center py-10 md:py-20 overflow-y-auto no-scrollbar pb-32">
          {renderStepContent()}
      </div>
              ...
              <div className="hidden md:flex flex-1 max-w-md px-10">
                  <div className="w-full h-1.5 rounded-full bg-gray-100 dark:bg-gray-800 relative">
                      <div 
                        ...
                        style={{ width: `${(Math.max(0, step) / totalSteps) * 100}%` }}
                      ></div>
                  </div>
              </div>
